...
class UserController {
  ...
  static login(req, res, next) {
    ...
    User.findOne({
      ...
      const access_token = generateToken({ id: user.id, fullname:user.fullname, email: user.email, role: user.role })
      res.status(200).json({ username: user.username, email: user.email, access_token})
    }).catch(err => next(err))
  }
