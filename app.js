...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', function(data) { // isinya { roomid: '', playerData }
  ...
  socket.on('join-room', function(data) { // isinya { roomid: '', playerData }
    console.log(data.roomid, 'ini roomid nya joinroom')
    const selectedRoom = activeRooms.find((datum => datum.roomid === data.roomid))
    if (selectedRoom) {
